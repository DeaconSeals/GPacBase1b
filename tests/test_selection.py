...
class TestUniformRandomParentSelection:
	def test_output_size(self):
		for _ in range(iterations):
			pop = random_pop(popsize, boardsize)
			outsize = random.randint(1, popsize * 2)
			assert len(sel.uniform_random_selection(pop, outsize)) == outsize
...
class TestFitnessProportionate:
	...
	#moderately high fitness means moderate bias
	def test_slightly_prefers_weak_outliers(self):
		outsize = popsize * 3
		min_fit = -10
		max_fit = 10
		big_fit = max_fit + ((max_fit - min_fit) * 30)
		failures = 0
		for _ in range(iterations):
			pop = random_pop(popsize, boardsize)
			random_fitness(pop, min_fit, max_fit)
			...
			for ind in sel.fitness_proportionate_selection(pop, outsize):
				if ind.fitness > max_fit:
					it += 1
			if it < outsize * 0.2 or it > outsize * 0.5:
				failures += 1
		assert failures < max(iterations/10, 2)
